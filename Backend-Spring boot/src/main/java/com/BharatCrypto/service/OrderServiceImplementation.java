...
import com.BharatCrypto.domain.OrderStatus;
import com.BharatCrypto.domain.OrderType;
import com.BharatCrypto.model.*;
import com.BharatCrypto.repository.*;

import com.BharatCrypto.repository.OrderRepository;
...
public class OrderServiceImplementation implements OrderService {
    ...
    public Order createOrder(User user, OrderItem orderItem, OrderType orderType) {
        ...
        double price = Double.parseDouble(orderItem.getCoin().getCurrentPrice())*orderItem.getQuantity();
    ...
    private OrderItem createOrderItem(Coin coin,double quantity,double buyPrice,double sellPrice) {
        // This method now only creates the object in memory, it does not save it.
    ...
    public Order buyAsset(Coin coin,double quantity, User user) throws Exception {
        ...
        double buyPrice=Double.parseDouble(coin.getCurrentPrice());

        OrderItem orderItem = createOrderItem(coin,quantity,buyPrice,0);

        Order order = createOrder(user, orderItem, OrderType.BUY);
        // Establish the bidirectional link before saving
    ...
    public Order sellAsset(Coin coin,double quantity, User user) throws Exception {
        double sellPrice =Double.parseDouble(coin.getCurrentPrice());
        ...
        if (assetToSell != null) {
            ...
            if (assetToSell.getQuantity() >= quantity) {
                ...
                if(updatedAsset.getQuantity()*Double.parseDouble(coin.getCurrentPrice())<=1){
