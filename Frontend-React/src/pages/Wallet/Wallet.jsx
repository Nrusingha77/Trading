...
const Wallet = () => {
  ...
  // ✅ Read from query params ONLY (Razorpay sends as ?order_id=X&razorpay_payment_id=Y)
  const orderId = query.get("order_id");
  const razorpayPaymentId = query.get("razorpay_payment_id");

  const depositTriggeredRef = useRef(false);

  // ✅ Deposit effect - runs when both order_id and payment_id are present
  useEffect(() => {
    console.log("🔍 Checking deposit trigger - orderId:", orderId, "paymentId:", razorpayPaymentId);

    if (orderId && razorpayPaymentId && !depositTriggeredRef.current) {
      ...
      dispatch(
        ...
        .then(() => {
          ...
          // Clean URL so deposit doesn't re-trigger on page refresh
          setTimeout(() => {
        ...
        .catch((error) => {
          ...
          depositTriggeredRef.current = false; // Reset so user can retry
        });
    }
  }, [orderId, razorpayPaymentId, dispatch, navigate]);

  // ✅ Initial wallet and transaction load
  useEffect(() => {
    dispatch(getUserWallet(localStorage.getItem("jwt")));
    dispatch(getWalletTransactions({ jwt: localStorage.getItem("jwt") }));
